...
public interface LandRepositoryJPA extends JpaRepository<Land, String> {
    ...
    @Query(value = "select distinct new com.atp.bdss.entities.Land( " +
            ...
            "CASE WHEN :#{#request.price} = 1 THEN (cast(l.price AS java.math.BigDecimal) * l.acreage) END DESC, " +
            "CASE WHEN :#{#request.price} = 0 THEN (cast(l.price AS java.math.BigDecimal) * l.acreage) END ASC")
